...
public partial class MainWindow
{
    ...
    {
        // Match the Qt/WinForms transient-shape path so validation does not
        // rebuild layer state or alter the current view extent.
        viewerControl.ClearShapes();
        viewerControl.AddPolygonShape(ValidPolygon, PolygonStyle("#BFD7EA", "#2F80C2"));
        viewerControl.AddPolygonShape(SelfIntersectingPolygon, PolygonStyle("#F6D6AD", "#D95D39"));
        ...
        if (checkedShape)
        {
            ...
            viewerControl.AddPolygonShape(ValidPolygon, CheckedStyle("#CDE7D8", "#2A9D8F"));
            viewerControl.AddPolygonShape(SelfIntersectingPolygon, CheckedStyle("#F4A261", "#D62828"));
