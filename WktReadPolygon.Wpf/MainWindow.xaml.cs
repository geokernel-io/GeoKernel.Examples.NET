...
public partial class MainWindow
{
    ...
    private void ReadPolygon_Click(object sender, RoutedEventArgs e) => ParseAndRender();
    ...
    private void ModeComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        readButton.Content = IsMultiPolygon ? "Read MultiPolygon" : "Read Polygon";
        ResetWkt();
        if (_loaded)
            ParseAndRender();
    }
    ...
    private void ParseAndRender()
    {
        ...
        try
        {
            var lonLatRings = viewerControl.ReadWktPolygon(input, IsMultiPolygon);
            ...
            viewerControl.AddPolygonLayer("WKT Polygon", webMercatorRings, PolygonStyle());
            viewerControl.ViewExtent = viewExtent;

            detailsTextBox.Text = DetailsText(input, apiName, lonLatRings, viewExtent);
            statusText.Text = $"{apiName} parsed {lonLatRings.Count} ring(s) and {VertexCount(lonLatRings)} vertices.";
        }
