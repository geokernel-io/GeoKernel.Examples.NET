...
internal static class SampleData
{
    public static async Task<string> EnsureFileAsync(string archiveName, string folderName, string fileName, string displayName, IWin32Window owner, IProgress<SampleDataProgress>? progress = null)
    {
        ...
        catch (Exception ex)
        {
            ...
            MessageBox.Show(owner, $"Sample data could not be prepared.{Environment.NewLine}{Environment.NewLine}{ex.Message}", "SnappingEnabled", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return string.Empty;
        }
    }
}
