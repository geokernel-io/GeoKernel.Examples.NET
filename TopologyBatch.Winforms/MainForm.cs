...
public sealed partial class MainForm : Form
{
    ...
    private void RenderScene(bool runBatch)
    {
        ...
        for (var i = 0; i < SourcePolygons.Length; ++i)
        {
            ...
            if (isValid)
            {
                ...
                geoKernelViewerControl.AddPolygonShape(polygon, SourceStyle(i, validated: true));
            }
            else
            {
                invalidCount++;
                geoKernelViewerControl.AddPolygonShape(polygon, InvalidStyle());
            }
        }
        ...
        var unionParts = geoKernelViewerControl.UnionPolygonsOnList(validPolygons);
        foreach (var part in unionParts)
            geoKernelViewerControl.AddPolygonShape(part, UnionStyle());
        stopwatch.Stop();
        ...
        statusLabel.Text = unionParts.Count > 0
    ...
    {
        for (var i = 0; i < SourcePolygons.Length; ++i)
            geoKernelViewerControl.AddPolygonShape(SourcePolygons[i], SourceStyle(i, validated));
    }
