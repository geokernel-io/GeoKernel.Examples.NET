...
namespace GeoKernel.TabLoad.Wpf;
...
public sealed partial class MainWindow
{
    ...
    private void Window_Loaded(object? sender, RoutedEventArgs e)
    {
        ...
        var path = SampleData.EnsureKnownWpfSampleFile("paris.tab", this);
        if (string.IsNullOrEmpty(path))
        {
            statusText.Text = "MapInfo TAB sample data could not be prepared.";
            return;
        }
        ...
        {
            statusText.Text = "MapInfo TAB could not be opened.";
            MessageBox.Show(this, $"MapInfo TAB could not be opened:{Environment.NewLine}{ex.Message}",
                "TabLoad", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
    ...
    private void PopulateDetails(string path)
    {
        ...
        statusText.Text = $"GisLayerTAB opened {layer.FeatureCount} memory features and {definitions.Count} fields.";
    }
