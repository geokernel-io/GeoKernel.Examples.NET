...
public partial class MainWindow
{
    ...
    private void ViewerControl_MapMouseUp(object? sender, GeoKernelMapMouseEventArgs e)
    {
        ...
        var worldPoint = viewerControl.ScreenToWorld(e.ScreenPoint.X, e.ScreenPoint.Y);
        ...
        viewerControl.SelectFeatureHit(hits[0], tolerance);
    ...
    private void HitsGrid_SelectionChanged(object sender, System.Windows.Controls.SelectionChangedEventArgs e)
    {
        ...
        viewerControl.SelectFeatureHit(_hits[index], _hitTolerance);
    ...
    private void ShowHits(IReadOnlyList<GeoKernelFeatureHitTestResult> hits)
    {
        ...
        {
            var hit = _hits[i];
            _hitRows.Add(new HitRow((i + 1).ToString(), hit.LayerName, hit.FeatureId.ToString(), hit.ShapeType.ToString(), BestDisplayName(hit)));
        }
    ...
    private sealed record HitRow(string Number, string Layer, string FeatureId, string ShapeType, string Display);
