...
public sealed partial class MainForm : Form
{
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "ToolSelect_Mode", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }

        if (!geoKernelViewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = style }))
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "ToolSelect_Mode", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
