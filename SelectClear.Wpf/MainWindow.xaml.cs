...
namespace GeoKernel.SelectClear.Wpf;
...
public partial class MainWindow
{
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "SelectClear", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }

        if (!viewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = style }))
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "SelectClear", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
    ...
    private void Add_Click(object sender, RoutedEventArgs e)
    {
        ...
        viewerControl.ActiveTool = GeoKernelViewerTool.Info;
        toolStateText.Text = "Click = addSelectedFeature";
        UpdateStatus("Click a feature to add it to selection.");
    }
    ...
    private void Clear_Click(object sender, RoutedEventArgs e)
    {
        ...
        UpdateStatus($"clearSelectedFeatures applied. selectedFeatureCount={viewerControl.SelectedFeatureCount}.");
    }
    ...
    private void ViewerControl_MapMouseUp(object? sender, GeoKernelMapMouseEventArgs e)
    {
        ...
        var ok = viewerControl.AddTopFeatureToSelectionAt(e.ScreenPoint.X, e.ScreenPoint.Y, 8);
        ...
        UpdateStatus($"addSelectedFeature applied. selectedFeatureCount={viewerControl.SelectedFeatureCount}.");
    }
