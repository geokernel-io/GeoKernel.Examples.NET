...
public sealed partial class MainForm : Form
{
    ...
    private bool LoadLayer()
    {
        ...
        geoKernelViewerControl.SetLayerName(0, WorldLayerName);
        return true;
    }
    ...
    private bool LoadWorkingLayer(bool beginEdit)
    {
        ...
        var info = geoKernelViewerControl.GetLayerInfo(geoKernelViewerControl.LayerCount - 1);
        if (info is not null)
        {
            _pointLayerIndex = info.Index;
            geoKernelViewerControl.SetLayerName(_pointLayerIndex, WorkingLayerName);
        }
    ...
    private void BeginPointEditing()
    {
        ...
        geoKernelViewerControl.SetNewFeatureAttributes(NewPointAttributes());
    }

    private void resetCopyButton_Click(object sender, EventArgs e)
    {
        CloseWorkingLayer();
    ...
    private void commitButton_Click(object sender, EventArgs e)
    {
        ...
        if (!geoKernelViewerControl.CommitEditLayer(_pointLayerIndex))
        {
    ...
    private void reloadButton_Click(object sender, EventArgs e)
    {
        CloseWorkingLayer();
        LoadWorkingLayer(beginEdit: true);
        geoKernelViewerControl.ActiveTool = addPointButton.Checked ? GeoKernelViewerTool.AddPoint : GeoKernelViewerTool.Pan;
        RefreshMap();
    ...
    {
        if (_pointLayerIndex >= 0 && geoKernelViewerControl.IsLayerEditing(_pointLayerIndex))
            geoKernelViewerControl.RollbackEditLayer(_pointLayerIndex);

        geoKernelViewerControl.RemoveLayerByName(WorkingLayerName);
        _pointLayerIndex = -1;
    }
    ...
    {
        var editing = _pointLayerIndex >= 0 && geoKernelViewerControl.IsLayerEditing(_pointLayerIndex);
        var dirty = _pointLayerIndex >= 0 && geoKernelViewerControl.IsLayerDirty(_pointLayerIndex);
        var count = FeatureCount();

        stateLabel.Text = $"Editing: {(editing ? "ON" : "OFF")} | Dirty: {(dirty ? "YES" : "NO")} | Feature count: {count}";
    ...
    {
        return _pointLayerIndex >= 0
            ? geoKernelViewerControl.GetLayerFeatureCount(_pointLayerIndex)
            : 0;
    }
