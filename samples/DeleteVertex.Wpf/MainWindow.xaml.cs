...
public partial class MainWindow
{
    ...
    private bool LoadLayer()
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"World shapefile could not be found:{Environment.NewLine}{path}", "DeleteVertex", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }

        if (!viewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = WorldStyle() }))
        {
            MessageBox.Show(this, $"World layer could not be loaded:{Environment.NewLine}{path}", "DeleteVertex", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
    ...
    private void DeleteSelectedVertex_Click(object sender, RoutedEventArgs e)
    {
        BeginEditing();
    ...
    private void DeleteByIndex_Click(object sender, RoutedEventArgs e)
    {
        BeginEditing();
        ...
        if (!viewerControl.DeleteSelectedFeatureVertexInEditLayer(partIndex, vertexIndex))
        {
    ...
    private void VertexIndexTextBox_TextChanged(object sender, System.Windows.Controls.TextChangedEventArgs e)
    {
        if (_loaded)
            UpdateInfo();
    }
    ...
    {
        var index = Math.Clamp(VertexIndex(), 0, Math.Max(0, _vertices.Count - 1));
        vertexIndexTextBox.Text = index.ToString(CultureInfo.InvariantCulture);
    ...
    {
        return int.TryParse(vertexIndexTextBox.Text, NumberStyles.Integer, CultureInfo.InvariantCulture, out var value)
            ? Math.Clamp(value, 0, Math.Max(0, _vertices.Count - 1))
            : 0;
    }
