...
{
    private static readonly string SampleName = "ShapefileSaveAs";
    ...
    private void LoadSample()
    {
        ...
        var details = new List<string> { "ShapefileSaveAs sample", "", "API", ApiText(), "" };
        try
        {
            RunSample(details);
            details.Add("");
            ...
            statusLabel.Text = "ShapefileSaveAs loaded.";
        }
        catch (Exception ex)
        {
            details.Add(ex.Message);
            detailsTextBox.Text = string.Join(Environment.NewLine, details);
            statusLabel.Text = "ShapefileSaveAs failed.";
            MessageBox.Show(this, ex.Message, "ShapefileSaveAs", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
