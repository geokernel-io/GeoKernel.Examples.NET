...
namespace GeoKernel.WkbRead.Winforms;
...
public sealed partial class MainForm : Form
{
    ...
    private void ParseAndRender()
    {
        ...
        try
        {
            ...
            statusLabel.Text = $"GisWkbReader::read parsed {GeometryName} from {wkb.Length} bytes.";
        }
        catch (Exception ex) when (ex is FormatException or ArgumentException)
        {
            detailsTextBox.Text = $"WKB parse failed:{Environment.NewLine}{ex.Message}";
            statusLabel.Text = "WKB parse failed.";
        }
    }
    ...
    private IReadOnlyList<IReadOnlyList<GeoKernelPoint>> ParseGeometry(byte[] wkb) =>
        ...
        {
            1 => [geoKernelViewerControl.ReadWkbLineString(wkb)],
            2 => geoKernelViewerControl.ReadWkbPolygon(wkb),
            3 => geoKernelViewerControl.ReadWkbPolygon(wkb, multiPolygon: true),
            _ => [[geoKernelViewerControl.ReadWkbPoint(wkb)]]
        };
    ...
    private void AddGeometryLayer(IReadOnlyList<IReadOnlyList<GeoKernelPoint>> parts)
    {
        ...
        {
            case 1:
                geoKernelViewerControl.AddPolylineLayer("WKB LineString", parts, LineStyle());
                break;
            case 2:
            case 3:
                geoKernelViewerControl.AddPolygonLayer("WKB Polygon", parts, PolygonStyle());
                break;
            default:
                geoKernelViewerControl.AddPointLayer("WKB Point", parts[0], PointStyle());
                break;
        }
    }
