...
public partial class MainWindow
{
    ...
    private void RenderScene(bool runBatch)
    {
        ...
        for (var i = 0; i < SourcePolygons.Length; ++i)
        {
            ...
            if (isValid)
            {
                ...
                viewerControl.AddPolygonLayer($"P{i + 1}", polygon, SourceStyle(i, validated: true));
            }
            else
            {
                invalidCount++;
                viewerControl.AddPolygonLayer($"P{i + 1} invalid", polygon, InvalidStyle());
            }
        }
        ...
        var resultLayerIndex = viewerControl.AddPolygonUnionOnListLayer(
            "Batch Union Result",
            validPolygons,
            UnionStyle());
        stopwatch.Stop();
    ...
    {
        for (var i = 0; i < SourcePolygons.Length; ++i)
            viewerControl.AddPolygonLayer($"P{i + 1}", SourcePolygons[i], SourceStyle(i, validated));
    }
