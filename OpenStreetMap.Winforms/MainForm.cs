...
namespace GeoKernel.OpenStreetMap.Winforms;

public sealed partial class MainForm : Form
{
    private static readonly string SampleName = "OpenStreetMap";
    private static readonly string SampleKind = "osm";
    public MainForm()
    {
        InitializeComponent();
    }
    ...
    private void LoadSample()
    {
        ...
        var details = new List<string> { "OpenStreetMap sample", "", "API", ApiText(), "" };
        try
        {
            RunSample(details);
            details.Add("");
            ...
            statusLabel.Text = "OpenStreetMap loaded.";
        }
        catch (Exception ex)
        {
            details.Add(ex.Message);
            detailsTextBox.Text = string.Join(Environment.NewLine, details);
            statusLabel.Text = "OpenStreetMap failed.";
            MessageBox.Show(this, ex.Message, "OpenStreetMap", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
