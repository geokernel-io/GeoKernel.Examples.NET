...
public sealed partial class MainForm : Form
{
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "InfoTool", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
        ...
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "InfoTool", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
    ...
    private void geoKernelViewerControl_MapMouseUp(object? sender, GeoKernelMapMouseEventArgs e)
    {
        ...
        if (hit is null || !hit.IsValid)
        {
            ...
            ShowMapClick(e.ScreenPoint.X, e.ScreenPoint.Y, worldPoint);
        ...
        ShowHit(e.ScreenPoint.X, e.ScreenPoint.Y, worldPoint, hit);
    ...
    private void ShowEmptyHit()
    {
        ...
        detailsGrid.Rows.Add("Tool", "GeoKernelViewerTool.Info");
        detailsGrid.Rows.Add("Click", "Click the map to inspect the top feature.");
    }
    ...
    private void ShowMapClick(double screenX, double screenY, GeoKernelPoint worldPoint)
    {
        detailsGrid.Rows.Clear();
        detailsGrid.Rows.Add("Tool", "GeoKernelViewerTool.Info");
        detailsGrid.Rows.Add("Event", "MapMouseUp while Info tool is active");
        detailsGrid.Rows.Add("Screen point", PointText(screenX, screenY));
        detailsGrid.Rows.Add("World point", PointText(worldPoint.X, worldPoint.Y));
        detailsGrid.Rows.Add("Hit", "No feature");
    }
    ...
    private void ShowHit(double screenX, double screenY, GeoKernelPoint worldPoint, GeoKernelFeatureHitTestResult hit)
    {
        detailsGrid.Rows.Clear();
        detailsGrid.Rows.Add("Tool", "GeoKernelViewerTool.Info");
        detailsGrid.Rows.Add("Event", "MapMouseUp + HitTestTopFeatureAt");
        detailsGrid.Rows.Add("Screen point", PointText(screenX, screenY));
        detailsGrid.Rows.Add("World point", PointText(worldPoint.X, worldPoint.Y));
