...
public sealed partial class MainForm : Form
{
    ...
    private void RenderScene(bool showResult)
    {
        ...
        var sourceLayerIndex = geoKernelViewerControl.AddPolygonLayer("Source: valid part + loop", SourceRings, SourceStyle());
        if (sourceLayerIndex < 0)
        {
            detailsTextBox.Text = "Source polygon could not be created.";
            statusLabel.Text = "Source polygon creation failed.";
            return;
        }
        ...
        else
        {
            ...
            if (layerIndex < 0)
            {
                ...
                statusLabel.Text = "FindAndDeleteLoops failed.";
                return;
            }

            var resultPartCount = geoKernelViewerControl.GetLayerFeatureCount(layerIndex);
            var resultExtent = geoKernelViewerControl.GetLayerProjectedExtent(layerIndex);
