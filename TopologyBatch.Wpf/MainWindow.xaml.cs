...
public partial class MainWindow
{
    ...
    private void RunBatch_Click(object sender, RoutedEventArgs e) => RenderScene(runBatch: true);
    ...
    private void RenderScene(bool runBatch)
    {
        ...
        if (!runBatch)
        {
            ...
            statusText.Text = "Source polygons are ready. Click Run Batch.";
        ...
        for (var i = 0; i < SourcePolygons.Length; ++i)
        {
            ...
            var isValid = viewerControl.CheckPolygonRing(polygon);
            ...
            if (isValid)
            {
                ...
                viewerControl.AddPolygonShape(polygon, SourceStyle(i, validated: true));
            }
            else
            {
                invalidCount++;
                viewerControl.AddPolygonShape(polygon, InvalidStyle());
            }
        }
        ...
        var unionParts = viewerControl.UnionPolygonsOnList(validPolygons);
        foreach (var part in unionParts)
            viewerControl.AddPolygonShape(part, UnionStyle());
        stopwatch.Stop();

        details.Add("");
        details.Add("Union result:");
        details.Add(unionParts.Count > 0 ? $"Result parts: {unionParts.Count}" : "Result: empty");
        details.Add($"Elapsed: {stopwatch.ElapsedMilliseconds} ms");

        statusText.Text = unionParts.Count > 0
    ...
    {
        for (var i = 0; i < SourcePolygons.Length; ++i)
            viewerControl.AddPolygonShape(SourcePolygons[i], SourceStyle(i, validated));
    }

    private void SetSampleExtent()
    {
        viewerControl.ViewExtent = new GeoKernelExtent(-6.5, -3.0, 5.8, 3.6);
    }
