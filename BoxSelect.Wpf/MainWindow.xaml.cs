...
public partial class MainWindow
{
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "BoxSelect", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }

        if (!viewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = style }))
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "BoxSelect", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
    ...
    private void ViewerControl_MapSelectionBoxFinished(object? sender, GeoKernelSelectionBoxFinishedEventArgs e)
    {
        ...
        var hits = viewerControl.GetSelectedFeatures()
