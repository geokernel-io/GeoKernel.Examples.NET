...
public sealed partial class MainForm : Form
{
    ...
    private void RenderScene(bool showResult)
    {
        ...
        geoKernelViewerControl.AddPolylineShape(SplitArc, ArcStyle());
        ...
        if (showResult)
        {
            var pieces = geoKernelViewerControl.SplitPolygonByArc(SourcePolygon, SplitArc);
            details += $"{Environment.NewLine}Result shapes: {pieces.Count}";
            for (var index = 0; index < pieces.Count; index++)
            {
                var piece = pieces[index];
                geoKernelViewerControl.AddPolygonShape(piece, ResultStyle(index));
                details += $"{Environment.NewLine}Piece {index + 1} parts: 1 extent: {ExtentText(piece)}";
            }

            geoKernelViewerControl.AddPolylineShape(SplitArc, ArcStyle());
            statusLabel.Text = pieces.Count > 0
