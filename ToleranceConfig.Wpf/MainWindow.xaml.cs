...
public partial class MainWindow
{
    ...
    {
        var tolerance = Math.Round(toleranceSlider.Value) / 100.0;
        ...
        // Use transient shapes like the WinForms/Qt sample. Recreating layers
        // for every slider step invalidates extent and render caches and flashes.
        viewerControl.ClearShapes();
        if (tolerance > 0)
            viewerControl.AddPolygonShape(ToleranceCircle(TestPoint, tolerance), ToleranceStyle(active));

        viewerControl.AddPolylineShape(Baseline, LineStyle());
        viewerControl.AddPointShape(TestPoint, PointStyle(active));

        foreach (var crossing in info.Crossings)
            viewerControl.AddPointShape(crossing, CrossingStyle());
