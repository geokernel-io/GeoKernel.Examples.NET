...
public sealed partial class MainForm : Form
{
    ...
    private void RenderScene()
    {
        ...
        // Match the Qt sample's transient-shape rendering path. Recreating
        // complete layers for every TrackBar step invalidates the layer and
        // extent caches and produces a visible flash.
        geoKernelViewerControl.ClearShapes();
        if (tolerance > 0)
            geoKernelViewerControl.AddPolygonShape(ToleranceCircle(TestPoint, tolerance), ToleranceStyle(active));

        geoKernelViewerControl.AddPolylineShape(Baseline, LineStyle());
        geoKernelViewerControl.AddPointShape(TestPoint, PointStyle(active));

        foreach (var crossing in info.Crossings)
            geoKernelViewerControl.AddPointShape(crossing, CrossingStyle());
