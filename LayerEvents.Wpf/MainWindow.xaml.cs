...
public partial class MainWindow : Window
{
    ...
    private void Window_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        AddLayer("States", "usa_states.shp", StatesStyle());
        AddLayer("Cities", "usa_cities.shp", CitiesStyle());
        RefreshLayerList();
    ...
    private void ConnectLayerEvents()
    {
        ...
        viewerControl.LayerAdded += (_, e) => OnUi(() => AppendLog($"Event: LayerAdded({LayerText(e)})"));
        viewerControl.LayerRemoved += (_, e) => OnUi(() => AppendLog($"Event: LayerRemoved({LayerText(e)})"));
        viewerControl.LayerVisibilityChanged += (_, e) =>
            OnUi(() => AppendLog($"Event: LayerVisibilityChanged(index={e.LayerIndex}, visible={e.Visible})"));
        viewerControl.LayerEditStateChanged += (_, e) => OnUi(() => AppendLog($"Event: LayerEditStateChanged({LayerText(e)})"));
        viewerControl.LayerEditSessionStarted += (_, e) => OnUi(() => AppendLog($"Event: LayerEditSessionStarted({LayerText(e)})"));
        viewerControl.LayerEditSessionCommitted += (_, e) => OnUi(() => AppendLog($"Event: LayerEditSessionCommitted({LayerText(e)})"));
        viewerControl.LayerEditSessionRolledBack += (_, e) => OnUi(() => AppendLog($"Event: LayerEditSessionRolledBack({LayerText(e)})"));
        viewerControl.LayerOrderChanged += (_, _) => OnUi(() => AppendLog("Event: LayerOrderChanged()"));
        viewerControl.BusyChanged += (_, e) => OnUi(() => AppendLog($"Event: BusyChanged({e.Busy})"));
        viewerControl.ViewChanged += (_, _) => OnUi(UpdateStatus);
    }
