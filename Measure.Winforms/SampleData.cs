...
{
    public static async Task<string> EnsureSampleFileAsync(Uri sourceUrl, string archiveName, string extractFolderName,
        string requiredFileName, string displayName, IWin32Window owner, IProgress<SampleDataProgress>? progress = null)
    {
        ...
        try
        {
            ...
            await using (var archive = File.Create(archivePath))
            {
                ...
                while (true)
                {
                    ...
                    progress?.Report(new SampleDataProgress(percentage.HasValue ? $"Downloading {displayName}... {percentage}%" : $"Downloading {displayName}...", percentage));
                }
            }
            ...
            if (!File.Exists(requiredPath)) throw new FileNotFoundException("The expected sample file was not found after extraction.", requiredPath);
            return requiredPath;
        }
        catch (Exception exception)
        {
            TryDelete(archivePath);
            MessageBox.Show(owner, $"Sample data could not be prepared.{Environment.NewLine}{Environment.NewLine}{exception.Message}", "Measure", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return string.Empty;
        }
    }
