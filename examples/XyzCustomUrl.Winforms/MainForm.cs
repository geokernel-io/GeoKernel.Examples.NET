...
{
    private static readonly string SampleName = "XyzCustomUrl";
    ...
    private void LoadSample()
    {
        ...
        var details = new List<string> { "XyzCustomUrl sample", "", "API", ApiText(), "" };
        try
        {
            RunSample(details);
            details.Add("");
            ...
            statusLabel.Text = "XyzCustomUrl loaded.";
        }
        catch (Exception ex)
        {
            details.Add(ex.Message);
            detailsTextBox.Text = string.Join(Environment.NewLine, details);
            statusLabel.Text = "XyzCustomUrl failed.";
            MessageBox.Show(this, ex.Message, "XyzCustomUrl", MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
