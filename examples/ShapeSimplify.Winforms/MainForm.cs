...
public sealed partial class MainForm : Form
{
    ...
    private void RenderScene()
    {
        ...
        geoKernelViewerControl.AddPolygonLayer($"source: {SourcePolygon.Length} vertices", SourcePolygon, SourceStyle());

        var simplified = geoKernelViewerControl.SimplifyPolygonRing(SourcePolygon, tolerance);
        if (simplified.Count > 0)
            geoKernelViewerControl.AddPolygonLayer($"simplified: tolerance {tolerance:F2}", simplified, ResultStyle());
