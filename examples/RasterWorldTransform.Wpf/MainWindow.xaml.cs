...
namespace GeoKernel.RasterWorldTransform.Wpf;

public sealed partial class MainWindow
{
    private static readonly string SampleName = "RasterWorldTransform";
    ...
    private void LoadSample()
    {
        ...
        var details = new List<string> { "RasterWorldTransform sample", "", "API", ApiText(), "" };
        try
        {
            RunSample(details);
            details.Add("");
            ...
            statusText.Text = "RasterWorldTransform loaded.";
        }
        catch (Exception ex)
        {
            details.Add(ex.Message);
            detailsTextBox.Text = string.Join(Environment.NewLine, details);
            statusText.Text = "RasterWorldTransform failed.";
            MessageBox.Show(this, ex.Message, "RasterWorldTransform", MessageBoxButton.OK, MessageBoxImage.Error);
        }
    }
    ...
    private void RunSample(List<string> details)
    {
        switch (SampleKind)
        {
            ...
            case "labelCollision":
                ...
                break;
            default:
                AddFile(details, "usa_3857.tif", null);
                break;
        }
    }
