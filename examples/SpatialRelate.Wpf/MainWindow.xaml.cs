...
public partial class MainWindow
{
    ...
    private void RunRelate_Click(object sender, RoutedEventArgs e)
    {
    ...
    private void RenderScene(bool showRelate)
    {
        ...
        viewerControl.AddPolygonLayer("Polygon A", PolygonA, StyleA());
        viewerControl.AddPolygonLayer("Polygon B", PolygonB, StyleB());
        ...
        if (showRelate)
        {
            ...
            {
                var matched = viewerControl.RelatePolygonRings(PolygonA, PolygonB, pattern);
                details += $"{Environment.NewLine}{name} ({pattern}): {matched.ToString().ToLowerInvariant()}";
            }

            statusText.Text = $"Relate matrix calculated: {matrix}";
        }
        else
        {
            details += $"{Environment.NewLine}{Environment.NewLine}Click Run Relate to calculate the relation matrix.";
            statusText.Text = "Source polygons are ready. Click Run Relate.";
        }
    ...
    {
        viewerControl.ViewExtent = new GeoKernelExtent(-5.1, -3.0, 5.0, 3.0);
    }
