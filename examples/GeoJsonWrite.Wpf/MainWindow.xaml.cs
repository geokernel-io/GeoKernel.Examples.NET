    ...
    private void RenderSelectedGeometry()
    {
        ...
        switch (mode)
        {
            case GeometryMode.Point:
                ...
                geoJson = viewerControl.WriteGeoJsonPoint(Point);
                var webPoint = ToWebMercator(Point);
                viewerControl.AddPointLayer("GeoJSON Point", [webPoint], PointStyle());
                viewExtent = PaddedExtent([webPoint]);
                break;
            case GeometryMode.Polyline:
                apiName = "GisGeoJsonWriter::writePolylineString(shape)";
                geoJson = viewerControl.WriteGeoJsonLineString(Polyline);
                var webLine = Polyline.Select(ToWebMercator).ToArray();
                viewerControl.AddPolylineLayer("GeoJSON Polyline", webLine, LineStyle());
                viewExtent = PaddedExtent(webLine);
                break;
            default:
                apiName = "GisGeoJsonWriter::writePolygonString(shape)";
                geoJson = viewerControl.WriteGeoJsonPolygon(Polygon);
                var webRing = Polygon.Select(ToWebMercator).ToArray();
                viewerControl.AddPolygonLayer("GeoJSON Polygon", webRing, PolygonStyle());
        ...
        statusText.Text = $"{apiName} wrote {((ComboBoxItem)geometryComboBox.SelectedItem).Content} GeoJSON.";
    }
