...
public sealed partial class MainForm : Form
{
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "SelectAdd", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }

        if (!geoKernelViewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = style }))
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "SelectAdd", MessageBoxButtons.OK, MessageBoxIcon.Error);
            return false;
        }
    ...
    private void geoKernelViewerControl_MouseClick(object? sender, MouseEventArgs e)
    {
        ...
        var ok = _selectionMode == SelectionMode.Toggle
            ? geoKernelViewerControl.ToggleTopFeatureSelectionAt(e.X, e.Y, 8)
