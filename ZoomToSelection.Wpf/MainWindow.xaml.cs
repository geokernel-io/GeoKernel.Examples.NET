...
namespace GeoKernel.ZoomToSelection.Wpf;
...
public partial class MainWindow
{
    ...
    private void Window_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        viewerControl.MapMouseUp += ViewerControl_MapMouseUp;
    ...
    private bool AddLayer(string fileName, string displayName, GeoKernelLayerStyle style)
    {
        ...
        if (!File.Exists(path))
        {
            MessageBox.Show(this, $"Sample data was not found:{Environment.NewLine}{path}", "ZoomToSelection", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }

        if (!viewerControl.AddLayerFile(path, new GeoKernelLayerLoadOptions { ApplyDefaultStyle = true, DefaultStyle = style }))
        {
            MessageBox.Show(this, $"Layer could not be loaded:{Environment.NewLine}{path}", "ZoomToSelection", MessageBoxButton.OK, MessageBoxImage.Error);
            return false;
        }
    ...
    private void ZoomToSelection_Click(object sender, RoutedEventArgs e)
    {
        ...
        if (viewerControl.ZoomToSelectedFeatures())
    ...
    private void ViewerControl_MapMouseUp(object? sender, GeoKernelMapMouseEventArgs e)
    {
        ...
        var ok = _selectionMode == SelectionMode.Toggle
            ? viewerControl.ToggleTopFeatureSelectionAt(e.ScreenPoint.X, e.ScreenPoint.Y, 8)
            : viewerControl.AddTopFeatureToSelectionAt(e.ScreenPoint.X, e.ScreenPoint.Y, 8);
        ...
        UpdateStatus(_selectionMode == SelectionMode.Toggle
            ? $"toggleSelectedFeature applied. selectedFeatureCount={viewerControl.SelectedFeatureCount}."
            : $"addSelectedFeature applied. selectedFeatureCount={viewerControl.SelectedFeatureCount}.");
    }
